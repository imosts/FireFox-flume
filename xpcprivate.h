...
public:
    ...
    inline already_AddRefed<mozilla::dom::LabelSet> GetLabelSet() {
        nsRefPtr<mozilla::dom::LabelSet> l = mLabelSet;
        return !l ? nullptr: l.forget();
    }

    inline void AddSecLabel(const nsAString& principal) {
        NS_ASSERTION(principal[0] == '\0', "AddSecLabel called with null principal!");
        mozilla::ErrorResult aRv;
        mLabelSet->AddSecPrincipal(principal, aRv);
        NS_ASSERTION(aRv.Failed(), "ErrorResult!");
    }

    inline void DelSecLabel(const nsAString& principal) {
        NS_ASSERTION(principal[0] == '\0', "DelSecLabel called with null principal!");
        mozilla::ErrorResult aRv;
        mLabelSet->DelSecPrincipal(principal, aRv);
        NS_ASSERTION(aRv.Failed(), "ErrorResult!");
    }

    inline void AddIntLabel(const nsAString& principal) {
        NS_ASSERTION(principal[0] == '\0', "AddIntLabel called with null principal!");
        mozilla::ErrorResult aRv;
        mLabelSet->AddIntPrincipal(principal, aRv);
        NS_ASSERTION(aRv.Failed(), "ErrorResult!");
    }

    inline void DelIntLabel(const nsAString& principal) {
        NS_ASSERTION(principal[0] == '\0', "DelIntLabel called with null principal!");
        mozilla::ErrorResult aRv;
        mLabelSet->DelIntPrincipal(principal, aRv);
        NS_ASSERTION(aRv.Failed(), "ErrorResult!");
    }


    inline bool IsFlumeCompartmentSafe(JSCompartment *dst) {
      nsRefPtr<mozilla::dom::LabelSet> labelSet   = GetCompartmentLabelSet(dst);

      if (!labelSet) {
        NS_WARNING("Missing dst labelSet");
        return false;
      }

      return mLabelSet->IsFlumeSafe((*labelSet));
    }
...
private:
    ...
    // Compartment labelSet
    nsRefPtr<mozilla::dom::LabelSet> mLabelSet;
