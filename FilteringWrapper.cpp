...
#define CFLUME FilteringWrapper<CrossCompartmentSecurityWrapper, ConfinementPolicy>
#define XFLUME FilteringWrapper<CrossOriginXrayWrapper, ConfinementPolicy>

template<> const XOW XOW::singleton(0);
template<> const NNXOW NNXOW::singleton(0);
template<> const NNXOWC NNXOWC::singleton(0);
template<> const CFLUME CFLUME::singleton(0);
template<> const XFLUME XFLUME::singleton(0);

template class XOW;
template class NNXOW;
template class NNXOWC;
template class ChromeObjectWrapperBase;
template class CFLUME;
template class XFLUME;
}
